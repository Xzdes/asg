# EdgeType — Типы рёбер в ASG

В ASG каждое ребро графа (ASG) имеет тип `EdgeType`, который определяет его семантику.  
...
🎯 Теперь вы знаете все типы рёбер, которые можно использовать в ASG в ASG!