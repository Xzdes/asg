# NodeType — Типы узлов в ASG

В ASG каждый узел графа (ASG) имеет тип `NodeType`, который определяет его семантику.  
...
🎯 Теперь вы знаете все типы узлов, которые можно использовать в ASG в ASG.
```
...
* Подробно расписаны все типы узлов ASG.
