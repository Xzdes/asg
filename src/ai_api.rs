...
pub fn export_asg_to_json(asg: &ASG) -> ASGResult<String> {
    serde_json::to_string_pretty(asg)
        .map_err(|e| ASGError::SerializationError(format!("Failed to export ASG to JSON: {}", e)))
}

/// Импортировать ASG из JSON-строки.
pub fn import_asg_from_json(json_str: &str) -> ASGResult<ASG> {
    serde_json::from_str(json_str)
        .map_err(|e| ASGError::SerializationError(format!("Failed to import ASG from JSON: {}", e)))
}

/// Сохранить ASG в файл JSON.
pub fn save_asg_to_file(asg: &ASG, path: &str) -> ASGResult<()> {
    let json = export_asg_to_json(asg)?;
    std::fs::write(path, json)
        .map_err(|e| ASGError::SerializationError(format!("Failed to write ASG to file: {}", e)))
}
