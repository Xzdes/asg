...
pub fn perform_io(input: &str) -> ASGResult<String> {
    println!("EffectIO: {}", input);
    Ok(input.to_string())
}

/// Эффект: вывод в консоль.
pub fn perform_console_output(message: &str) -> ASGResult<()> {
...
pub fn perform_fs_write(path: &str, data: &str) -> ASGResult<()> {
    let mut file = File::create(path)
        .map_err(|e| ASGError::Effect(format!("Failed to create file: {}", e)))?;
    file.write_all(data.as_bytes())
        .map_err(|e| ASGError::Effect(format!("Failed to write file: {}", e)))?;
    Ok(())
}
