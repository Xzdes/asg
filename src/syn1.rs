...
//!
//! Ошибки обрабатываются через ASGError::Serialization.
...
pub fn load_syn1(path: &str) -> ASGResult<ASG> {
    let mut file = File::open(path)
        .map_err(|e| ASGError::Serialization(format!("Failed to open file: {}", e)))?;

    let mut header = [0u8; 4];
    file.read_exact(&mut header)
        .map_err(|e| ASGError::Serialization(format!("Failed to read header: {}", e)))?;
    if &header != b"SYN1" {
        return Err(ASGError::Serialization("Invalid SYN1 header".into()));
    }

    let _version = file
        .read_u8()
        .map_err(|e| ASGError::Serialization(format!("Failed to read version: {}", e)))?;
    ...
    for _ in 0..node_count {
        ...
        let node_type = NodeType::try_from(node_type_code as u32)
            .map_err(|_| ASGError::Serialization(format!("Unknown NodeType: {}", node_type_code)))?;
        ...
        if payload_length > 0 {
            ...
            file.read_exact(&mut buf)
                .map_err(|e| ASGError::Serialization(format!("Failed to read node payload: {}", e)))?;
            payload = Some(buf);
        }
        ...
        for _ in 0..edge_count {
            ...
            let edge_type = EdgeType::try_from(edge_type_code as u32)
                .map_err(|_| ASGError::Serialization(format!("Unknown EdgeType: {}", edge_type_code)))?;
            ...
            if edge_payload_length > 0 {
                ...
                file.read_exact(&mut buf)
                    .map_err(|e| ASGError::Serialization(format!("Failed to read edge payload: {}", e)))?;
...
fn read_varint<R: Read>(reader: &mut R) -> ASGResult<u64> {
    let mut result = 0u64;
    let mut shift = 0u8;
    ...
    loop {
        let byte = reader
            .read_u8()
            .map_err(|e| ASGError::Serialization(format!("Failed to read varint: {}", e)))?;
        ...
        shift += 7;
        if shift >= 64 {
            return Err(ASGError::Serialization("VarInt too long".into()));
        }
    }
