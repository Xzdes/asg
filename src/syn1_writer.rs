...
pub fn save_syn1(asg: &ASG, path: &str) -> ASGResult<()> {
    let file = File::create(path)
        .map_err(|e| ASGError::Serialization(format!("Failed to create file: {}", e)))?;
    let mut writer = BufWriter::new(file);

    // Заголовок: "SYN1"
    writer
        .write_all(b"SYN1")
        .map_err(|e| ASGError::Serialization(format!("Failed to write header: {}", e)))?;

    // Версия (u8)
    writer
        .write_u8(1)
        .map_err(|e| ASGError::Serialization(format!("Failed to write version: {}", e)))?;
    ...
    for node in &asg.nodes {
        ...
        if let Some(payload) = &node.payload {
            ...
            writer
                .write_all(payload)
                .map_err(|e| ASGError::Serialization(format!("Failed to write node payload: {}", e)))?;
        } else {
        ...
        for edge in &node.edges {
            ...
            if let Some(edge_payload) = &edge.payload {
                ...
                writer
                    ...
                    .map_err(|e| ASGError::Serialization(format!("Failed to write edge payload: {}", e)))?;
            } else {
                write_varint(&mut writer, 0)?;
            }
        }
    ...
    writer.flush()
        .map_err(|e| ASGError::Serialization(format!("Failed to flush writer: {}", e)))?;

    Ok(())
}

/// Запись VarInt в поток.
fn write_varint<W: Write>(writer: &mut W, mut value: u64) -> ASGResult<()> {
    loop {
        ...
        writer
            .write_u8(byte)
            .map_err(|e| ASGError::Serialization(format!("Failed to write varint: {}", e)))?;
        if value == 0 {
            break;
        }
    }
